...
public class PersonView extends Div {
    ...
    public PersonView(PersonRepository personRepository) {
        ...
        this.formView = new FormView(this, personRepository);
...
//
    ...
    private Component createGrid() {
        ...
        grid.setItems(personRepository.findAll().toList());
//        grid.setItems(query -> {

//            var pageRequest = PageRequest.ofPage(query.getPage(), query.getPageSize(), true);
//
//            Order<Person> order = Order.by(List.of(Sort.asc("code")));
//
////            return personRepository.findAll(pageRequest, order).stream();
//            return personRepository.offSet(pageRequest).stream();
//        });


//        PageRequest pageRequest, Order<T > sortBy
